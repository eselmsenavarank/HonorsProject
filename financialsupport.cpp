...
void FinancialSupport::readFile(){ //exception handling and file input are here
    ...
    try
    {
        if(fsFile.open(QIODevice::Truncate | QIODevice::WriteOnly)){
            ...
            throw QString("File cannot open");
        }
    }

    catch (const QString &errorMessage){
        ui -> textLabel -> setText(errorMessage);
    }
}

void FinancialSupport::printFile(){ // file output is here
